...
export async function POST(request: Request) {
  try {
    ...
    const platforms = rawPlatforms.flatMap((entry) => { if (!entry || typeof entry !== "object") return []; const item=entry as Record<string,unknown>; const platform=String(item.platform??""), packageType=String(item.packageType??""); const downloadUrl=String(item.downloadUrl??"").trim(), repoUrl=String(item.repoUrl??"").trim(); const metadata=item.metadata&&typeof item.metadata==="object"?item.metadata as Record<string,unknown>:null; if (!["Android","Windows","Linux"].includes(platform) || !["apk","exe","msi","deb","rpm","appimage"].includes(packageType) || !downloadUrl) return []; try { const url=new URL(downloadUrl); if (url.protocol!=="https:"&&url.protocol!=="http:") return []; } catch { return []; } if(platform==="Android"&&packageType!=="apk")return []; if(platform==="Windows"&&!["exe","msi"].includes(packageType))return []; if(platform==="Linux"&&!["deb","rpm","appimage"].includes(packageType))return []; return [{platform,packageType,downloadUrl,...(repoUrl?{repoUrl}:{}),...(metadata?{metadata}: {})}]; });
