...
export default function LumaDeveloperPortal() {
  ...
  if (submitted) return (
    <div className="glass-page mx-auto max-w-3xl py-16 text-center"><div className={`${cardClass} p-10`}>
      ...
      <button onClick={() => { setSubmitted(false); resetForm(); }} className="mt-8 ui-button-primary px-5 py-2.5 font-medium text-white hover:bg-indigo-500">Back to apps</button>
    </div></div>
  );
...
https://.../screenshot2.png"/></div></div></div>
                ...
                <div className="ui-panel-muted p-5"><h3 className="font-semibold text-white">Developer funding</h3><p className="mt-1 text-sm text-slate-400">Funding is configured once for your developer profile and automatically applies to every app.</p><Link href="/dashboard/funding" className="mt-3 inline-flex ui-button-primary px-4 py-2 text-sm font-medium text-white">Manage developer funding</Link></div>
                ...
                {fastlaneMetadata&&<div className="ui-panel-muted p-5"><p className="text-sm font-semibold text-emerald-300">Store metadata ready</p><p className="mt-1 text-xs text-slate-500">{fastlaneMetadata.locale} · {fastlaneMetadata.screenshots.length} screenshots</p><p className="mt-4 text-lg font-semibold text-white">{fastlaneMetadata.title}</p></div>}
                <div className="flex justify-between">
                  <button type="button" onClick={()=>goToStep(1)} className="ui-button-secondary px-5 py-2.5 text-white">Back</button>
                  <button type="button" onClick={continueToReview} disabled={continueDisabled} className="ui-button-primary px-5 py-2.5 text-white disabled:opacity-40">Continue</button>
                </div>
              </div>}
