...
function rowToApp(item: LumaSubmissionRow): AppSubmission {
  return {
    ...
    separatePlatformRepos: item.separate_platform_repos === true || parsePlatformArtifacts(item.platforms, item.platform, item.download_url, item.linux_package_base).some((platform) => Boolean(platform.repoUrl || platform.metadata)),
...
export default function LumaDeveloperPortal() {
  ...
  const [developerBadgePlatform, setDeveloperBadgePlatform] = useState("all");
  ...
  const removeSubmission = async (app: AppSubmission) => {
    const action = app.status === "Approved" ? "archive" : "delete";
    if (!window.confirm(`${action === "archive" ? "Archive" : "Delete"} ${app.name}? ${action === "archive" ? "It will no longer be an active Store app." : "This cannot be undone."}`)) return;
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.access_token) return;
    const response = await fetch(`/api/luma/submissions?id=${encodeURIComponent(app.id)}`, { method: "DELETE", headers: { Authorization: `Bearer ${session.access_token}` } });
    if (!response.ok) { const body = await response.json().catch(() => ({})); alert(body.error || "Action failed."); return; }
    setMyApps((items) => action === "archive" ? items.map((item) => item.id === app.id ? { ...item, status: "Archived" } : item) : items.filter((item) => item.id !== app.id));
  };
  ...
  return (
    <div className="glass-page mx-auto max-w-6xl space-y-8 pb-20">
      ...
      <section className={cardClass}><div className="border-b border-white/10 px-6 py-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-500">Recent activity</p><h2 className="mt-1 font-semibold text-white">What changed</h2></div><div className="divide-y divide-white/10">{myApps.slice(0,6).map(app=>{const stats=submissionStoreIds[app.id]?downloadStats[submissionStoreIds[app.id]]:undefined;return <div key={app.id} className="flex items-start justify-between gap-4 px-6 py-4"><div><p className="text-sm font-medium text-white">{app.name}</p><p className="mt-1 text-xs text-slate-500">{app.status==="Approved"?"Published in Luma Store":app.status==="Changes Requested"?"Changes requested":app.status==="Rejected"?"Submission rejected":app.status==="Pending"||app.status==="In Review"?"Submission under review":"Submission "+app.status.toLowerCase()}</p></div><div className="text-right"><span className={`rounded-full border px-2 py-1 text-[11px] ${getStatusColor(app.status)}`}>{app.status}</span>{stats&&<p className="mt-2 text-xs text-slate-500">↓ {Number(stats.total||0).toLocaleString()}</p>}</div></div>})}{!loadingApps&&myApps.length===0&&<div className="px-6 py-8 text-center text-sm text-slate-500">Activity will appear here after your first submission.</div>}</div></section>

      <section className={cardClass}><div className="border-b border-slate-800 px-6 py-5"><h2 className="font-semibold text-white">Luma Store downloads</h2><p className="mt-1 text-xs text-slate-500">Combined across all of your published apps and all versions. Only files hosted by Luma Store are counted.</p></div><div className="p-6"><div className="rounded-xl border border-slate-800 bg-slate-950/45 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">All-time downloads</p><p className="mt-2 text-2xl font-bold text-white">{Object.values(downloadStats).reduce((sum,row)=>sum+Number(row.total||0),0).toLocaleString()}</p></div></div>{developerName&&<div className="border-t border-slate-800 px-6 py-5"><p className="mb-3 text-xs uppercase tracking-wide text-slate-500">README badge · all apps</p><div className="flex flex-wrap items-center gap-3"><select value={developerBadgePlatform} onChange={(e)=>setDeveloperBadgePlatform(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs text-slate-200"><option value="all">All platforms</option><option value="Android">Android</option><option value="Windows">Windows</option><option value="Linux">Linux</option></select><img src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/download-badge?developer_name=${encodeURIComponent(developerName)}&badge_v=3${developerBadgePlatform==="all"?"":`&platform=${encodeURIComponent(developerBadgePlatform)}`}`} alt="Luma Store total downloads" className="h-5 w-auto"/><button type="button" onClick={()=>{const platformPart=developerBadgePlatform==="all"?"":`&platform=${encodeURIComponent(developerBadgePlatform)}`;navigator.clipboard.writeText(`[![Luma Store total downloads](${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/download-badge?developer_name=${encodeURIComponent(developerName)}&badge_v=3${platformPart})](${window.location.origin}/discover/developers/${encodeURIComponent(developerName)})`)}} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs text-slate-300 hover:text-white">Copy badge Markdown</button></div></div>}</section>
          ...
          <section className={cardClass}>
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8">
              ...
              {step === 1 && <div className="space-y-6">
                ...
                <div className="flex justify-end"><button type="button" onClick={()=>goToStep(2)} disabled={!appPlatforms.length} className="rounded-xl bg-indigo-600 px-5 py-2.5 font-medium text-white hover:bg-indigo-500 disabled:opacity-40">Next</button></div>
              </div>}

              {step === 2 && <div className="space-y-6">
                <div><label className="flex items-center gap-3 text-sm font-medium text-slate-300"><input type="checkbox" checked={separatePlatformRepos} onChange={(e)=>{setSeparatePlatformRepos(e.target.checked);invalidateFastlane();}} className="h-4 w-4 accent-indigo-500"/>Different repository per platform</label><p className="mt-2 text-xs text-slate-500">Enable this when Android, Windows or Linux are maintained in different repositories. Each selected platform then requires its own repository and store metadata.</p></div>
                ...
                {separatePlatformRepos&&<div className="space-y-5">{appPlatforms.map(platform=>{const item=platformMetadata[platform];const update=(field:keyof PlatformMetadataInput,value:string)=>setPlatformMetadata(current=>({...current,[platform]:{...current[platform],[field]:value}}));return <div key={platform} className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-5"><h3 className="font-semibold text-white">{platform} repository & metadata</h3><p className="mt-1 text-xs text-slate-500">Required because this platform uses a separate repository.</p><div className="mt-4 grid gap-4"><input type="url" required value={item.repoUrl} onChange={e=>update("repoUrl",e.target.value)} className={fieldClass} placeholder="https://github.com/owner/repository"/><input required value={item.title} onChange={e=>update("title",e.target.value)} className={fieldClass} placeholder="Title"/><textarea required value={item.shortDescription} onChange={e=>update("shortDescription",e.target.value)} className={fieldClass} placeholder="Short description"/><textarea required rows={6} value={item.fullDescription} onChange={e=>update("fullDescription",e.target.value)} className={fieldClass} placeholder="Full description"/><textarea required rows={4} value={item.changelog} onChange={e=>update("changelog",e.target.value)} className={fieldClass} placeholder="Changelog"/><textarea required rows={4} value={item.screenshotsText} onChange={e=>update("screenshotsText",e.target.value)} className={fieldClass} placeholder="Screenshot URLs, one per line"/></div></div>})}</div>}
...
https://.../screenshot2.png"/></div></div></div>
                ...
                <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-5"><h3 className="font-semibold text-white">Developer funding</h3><p className="mt-1 text-sm text-slate-400">Funding is configured once for your developer profile and automatically applies to every app.</p><Link href="/dashboard/funding" className="mt-3 inline-flex rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white">Manage developer funding</Link></div>

                {!manualStoreMetadata && !separatePlatformRepos && <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4"><button type="button" onClick={verifyFastlane} disabled={!appLink.trim()||!validAndroidMetadata||fastlaneLoading} className="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-40">{fastlaneLoading?"Checking Fastlane…":"Check Fastlane metadata"}</button>{fastlaneError&&<p className="mt-3 text-sm text-red-400">{fastlaneError}</p>}</div>}
                ...
                <div className="flex justify-between"><button type="button" onClick={()=>goToStep(1)} className="rounded-xl bg-slate-800 px-5 py-2.5 text-white">Back</button><button type="button" onClick={()=>{if(separatePlatformRepos){const primary=platformMetadata[appPlatforms[0]];if(primary){const metadata={title:primary.title.trim(),shortDescription:primary.shortDescription.trim(),fullDescription:primary.fullDescription.trim(),changelog:primary.changelog.trim(),screenshots:primary.screenshotsText.split(/\\r?\\n/).map(value=>value.trim()).filter(Boolean),locale:"en-US",branch:"manual"};setFastlaneMetadata(metadata);setAppName(metadata.title);}}else if(manualStoreMetadata&&manualMetadataValid){setFastlaneMetadata({title:closedTitle.trim(),shortDescription:closedShortDescription.trim(),fullDescription:closedFullDescription.trim(),changelog:closedChangelog.trim(),screenshots:closedScreenshots,locale:"en-US",branch:"manual"});setAppName(closedTitle.trim());} goToStep(3);}} disabled={separatePlatformRepos?appPlatforms.some(platform=>{const item=platformMetadata[platform];return !item.repoUrl.trim()||!item.title.trim()||!item.shortDescription.trim()||!item.fullDescription.trim()||!item.changelog.trim()||!item.screenshotsText.trim()}):manualStoreMetadata?!manualMetadataValid:!fastlaneMetadata} className="rounded-xl bg-indigo-600 px-5 py-2.5 text-white disabled:opacity-40">Continue</button></div>
              </div>}
