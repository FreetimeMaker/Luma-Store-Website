          ...
          <section className={cardClass}>
            ...
            <form ref={formTopRef} onSubmit={handleSubmit} className="scroll-mt-20 p-4 sm:p-6 md:p-8">
              ...
              {step === 1 && <div className="space-y-6">
                ...
                <div className="flex justify-stretch sm:justify-end"><button type="button" onClick={()=>goToStep(2)} disabled={!appPlatforms.length} className="ui-button-primary w-full px-5 py-2.5 sm:w-auto font-medium text-white hover:bg-indigo-500 disabled:opacity-40">Next</button></div>
              </div>}
...
https://.../screenshot2.png"/></div></div></div>
                ...
                {fastlaneMetadata&&<div className="ui-panel-muted p-4 sm:p-5"><p className="text-sm font-semibold text-emerald-300">Store metadata ready</p><p className="mt-1 text-xs text-slate-500">{fastlaneMetadata.locale} · {fastlaneMetadata.screenshots.length} screenshots</p><p className="mt-4 text-lg font-semibold text-white">{fastlaneMetadata.title}</p></div>}
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
                  <button type="button" onClick={()=>goToStep(1)} className="ui-button-secondary w-full px-5 py-2.5 sm:w-auto text-white">Back</button>
                  <button type="button" onClick={continueToReview} disabled={continueDisabled} className="ui-button-primary w-full px-5 py-2.5 sm:w-auto text-white disabled:opacity-40">Continue</button>
                </div>
              </div>}
